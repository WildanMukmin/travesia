...
interface DestinasiEditAdminPageProps {
  destinasiId: string;
  destinasiData: GetOneDestinasiWithOwner;
}

const DestinasiEditAdminPage = ({
  destinasiId,
  destinasiData,
}: DestinasiEditAdminPageProps) => {
  if (!destinasiId) {
    return null;
  }
  ...
  const form = useForm<z.infer<typeof editDestinasiSchema>>({
    resolver: zodResolver(editDestinasiSchema),
    defaultValues: {
      namaDestinasi: destinasiData?.namaDestinasi,
      harga: destinasiData?.harga.toString(),
      deskripsi: destinasiData?.deskripsi,
      alamat: destinasiData?.alamat,
      nomorOwner: destinasiData?.nomorOwner,
      kategoriLokasi: destinasiData?.kategoriLokasi,
      jamOprasional: destinasiData?.jamOprasional,
      fasilitas: destinasiData?.fasilitas,
    },
  });

  const handleSubmitData = (data: z.infer<typeof editDestinasiSchema>) => {
    setErrorMessage("");
    setIsPending(true);
    data.image = imageFile;
    startTransition(() => {
      editDestinasi(destinasiId, data, true).then((res) => {
        if (res?.error) {
...
export default DestinasiEditAdminPage;
