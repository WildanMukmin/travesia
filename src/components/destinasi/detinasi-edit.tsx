...
const DestinasiEditPage = () => {
  ...
  const [srcImage, setSrcImage] = useState<string>(data?.image?.gambar || "");
  ...
  useEffect(() => {
    if (destinasiId) {
      getDestinasiById(destinasiId).then((res) => {
        if (res) {
          setData(res);
          setSrcImage(res.image?.gambar || "");
...
export default DestinasiEditPage;
